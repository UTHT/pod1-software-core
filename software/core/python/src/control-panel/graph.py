...
class Graph(QApplication):
    ...
    def plot(self, max, data) -> bool:  # "data" here represents the data read from the pod
        if not isinstance(data, float) or not isinstance(data, list) or not isinstance(max, int):
            return False
        self.x_data = np.arange(0, max)
        self.y_data = data  # chanel_name.handle(data_structure)
        return True

    def set_width(self, window_width) -> bool:
        if not isinstance(window_width, int):
            return False
        self.width = window_width
        return True

    def set_height(self, window_height) -> bool:
